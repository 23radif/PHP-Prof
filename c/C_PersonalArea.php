...
class C_PersonalArea extends C_Page
{
    public function action_index()
    {
        ...
        if (login()) {
            ...
            $this->content = "<h1>Добро пожаловать {$row['name']} в Ваш личный кабинет!</h1>
	...
	Ваш день рождения: <span style='color:gray;font-weight:600'>{$row['dob']}</span>";
            ...
            if (!isAdmin()) {
                ...
                $this->content .= "<h2><br>Мои товары: </h2>";
            } else {
                $this->content .= "<h2><br>Товары всех клиентов: </h2>";
            }
            ...
            while ($row = mysqli_fetch_assoc($res)) {
                ...
                $this->content .= "<br><br><h3>Заказ №{$row['id']} (идентификатор в базе данных)</h3>
                ...
                $this->content .= "<span>Общая сумма: {$sumPrice}</span><br> Статус: {$statusProduct}";
